! function(a) {
	...
	var b = function(b) {
		...
		function k(b) {
			h() || (c.input && c.input.length > 0 ? b.target !== c.input[0] && 0 === a(b.target).parents(".ys-picker-modal").length && c.close() : 0 === a(b.target).parents(".ys-picker-modal").length && c.close())
		}
		...
		return c.params = b, c.cols = [], c.initialized = !1, c.inline = c.params.container ? !0 : !1, f = a.device.ios || navigator.userAgent.toLowerCase().indexOf("safari") >= 0 && navigator.userAgent.toLowerCase().indexOf("chrome") < 0 && !a.device.android, c.setValue = function(a, b) {
			...
			var b = g();
			c.opened || (c.layout(), b ? (c.pickerHTML = '<div class="popover popover-picker-columns"><div class="popover-inner">' + c.pickerHTML + "</div></div>", c.popover = a.popover(c.pickerHTML, c.params.input, !0), c.container = a(c.popover).find(".ys-picker-modal"), a(c.popover).on("close", function() {
				l()
	...
	a(document).on("click", ".close-picker", function() {
		var b = a(".ys-picker-modal.ys-picker-modal-visible");
		b.length > 0 && a.closePicker(b)
	}), a(document).on(a.touchEvents.move, ".picker-modal-inner", function(a) {
		a.preventDefault()
	}), a.openPicker = function(b, c) {
		var d, e;
		return a.closePicker(), d = a("<div class='ys-picker-container " + (c || "") + "'></div>").appendTo(document.body), d.show(), d.addClass("ys-picker-container-visible"), e = a(b).appendTo(d), e.width(), e.addClass("ys-picker-modal-visible"), e
	}, a.closePicker = function() {
		a(".ys-picker-modal-visible").removeClass("ys-picker-modal-visible").transitionEnd(function() {
